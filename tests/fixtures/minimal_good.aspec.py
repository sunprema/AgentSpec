...
HOUSE_RULES = [
    ("Never modify anything outside the inbox directory",
     "the routine is a reader; writes belong to other routines"),
]
...
class TriageMessage(Task):
    ...
    constraints = HOUSE_RULES + [
        ("A message that merely mentions a product name is not automatically "
         "actionable", "name-dropping is the most common false positive"),
    ]
...
class InboxRoutine(Task):
    ...
    ticket = FileTicket(category=triage.category,
                        summary=triage.summary) if triage.actionable else None
